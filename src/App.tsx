...
import { Mail, Linkedin, Github, Briefcase, Code, User, Home, Award, ExternalLink, Brain, Sigma, Orbit } from 'lucide-react'; 
...
const projectsData: Project[] = [
  ...
  },
  {
    ...
    liveUrl: '#',
    repoUrl: 'https://github.com/ssshaise/Movie-Recommender',
    imageUrl: 'https://i.postimg.cc/2SVzcsQp/pexels-photo-7991579.jpg'
  },
  {
    id: 5,
...
const experienceData: Experience[] = [
  {
    ...
    description: [
      'Enhanced AI-driven systems for malware and cyber threat detection in email traffic at Ciscos Security Business Group in Prague.',
      'Assisted in dataset maintenance, analysis, and Python prototype development under senior engineer mentorship.',
      'Contributed to transforming prototypes into production-ready code, researching patentable ideas, and publishing conference papers.',
...
// CardContent, CardHeader, CardTitle, CardDescription (unchanged)
...
const AnimatedSection: React.FC<{ children: ReactNode; className?: string; delay?: number }> = ({ children, className = '', delay = 0 }) => {
  ...
  useEffect(() => { const section = sectionRef.current; if (!section) return; const observer = new IntersectionObserver( (entries) => { entries.forEach((entry) => { if (entry.isIntersecting) { setIsVisible(true); observer.unobserve(section); } }); }, { root: null, rootMargin: '0px', threshold: 0.1 } ); observer.observe(section); return () => { if (section) observer.unobserve(section); observer.disconnect(); }; }, [delay]);
  return ( <div ref={sectionRef} className={`transition-all duration-500 ease-out ${ isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10' } ${className}`} > {children} </div> );
};

// AnimatedHeading (unchanged)
const AnimatedHeading: React.FC<{ text: string; className?: string; wordDelay?: number }> = ({ text, className = '', wordDelay = 75 }) => {
  const [isVisible, setIsVisible] = useState(false); const headingRef = useRef<HTMLHeadingElement>(null); const words = text.split(' ');
  useEffect(() => { const heading = headingRef.current; if (!heading) return; const observer = new IntersectionObserver( (entries) => { entries.forEach((entry) => { if (entry.isIntersecting) { setIsVisible(true); observer.unobserve(heading); } }); }, { root: null, rootMargin: '0px', threshold: 0.5 } ); observer.observe(heading); return () => { if (heading) observer.unobserve(heading); observer.disconnect(); }; }, []);
  return ( <h2 ref={headingRef} className={` ${className}`}> {words.map((word, index) => ( <span key={index} className="inline-block overflow-hidden pb-1"> <span className={`inline-block transition-all duration-300 ease-out ${ isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-full' }`} style={{ transitionDelay: `${index * wordDelay}ms` }} > {word}&nbsp; </span> </span> ))} </h2> );
};

// CursorFollower (unchanged)
const CursorFollower: React.FC = () => {
  const [position, setPosition] = useState({ x: -100, y: -100 }); const [targetPosition, setTargetPosition] = useState({ x: -100, y: -100 }); const [isClicked, setIsClicked] = useState(false); const animationFrameRef = useRef<number | null>(null);
  const baseEasingFactor = 0.1; const distanceBoostFactor = 0.001; const maxEasingFactor = 0.5; const minDistanceThreshold = 1;
  useEffect(() => { const handleMouseMove = (event: MouseEvent) => { setTargetPosition({ x: event.clientX, y: event.clientY }); }; window.addEventListener('mousemove', handleMouseMove); return () => window.removeEventListener('mousemove', handleMouseMove); }, []);
  useEffect(() => { const handleMouseDown = () => setIsClicked(true); const handleMouseUp = () => setIsClicked(false); window.addEventListener('mousedown', handleMouseDown); window.addEventListener('mouseup', handleMouseUp); return () => { window.removeEventListener('mousedown', handleMouseDown); window.removeEventListener('mouseup', handleMouseUp); }; }, []);
  useEffect(() => { const animate = () => { setPosition((currentPos) => { const dx = targetPosition.x - currentPos.x; const dy = targetPosition.y - currentPos.y; const distance = Math.sqrt(dx * dx + dy * dy); if (distance < 0.1) return currentPos; let currentEasingFactor = baseEasingFactor; if (distance > minDistanceThreshold) { currentEasingFactor = Math.min( baseEasingFactor + distance * distanceBoostFactor, maxEasingFactor ); } const nextX = currentPos.x + dx * currentEasingFactor; const nextY = currentPos.y + dy * currentEasingFactor; return { x: nextX, y: nextY }; }); animationFrameRef.current = requestAnimationFrame(animate); }; animationFrameRef.current = requestAnimationFrame(animate); return () => { if (animationFrameRef.current) cancelAnimationFrame(animationFrameRef.current); }; }, [targetPosition]);
  return ( <div className={`fixed w-6 h-6 rounded-full bg-gray-400 dark:bg-gray-600 opacity-50 pointer-events-none z-[9999] transition-transform duration-100 ease-out ${isClicked ? 'scale-150' : 'scale-100'}`} style={{ left: `${position.x}px`, top: `${position.y}px`, transform: 'translate(-50%, -50%)', }} /> );
};

// HoverableText Component (unchanged)
    ...
    return (
        <Component className={className}>
            {characters.map((char, index) => (
                <span
                    key={index}
                    className="inline-block transition-colors duration-100 ease-in-out hover:text-gray-50 dark:hover:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-300"
                >
                    {char === ' ' ? '\u00A0' : char}
                </span>
            ))}
...
// TypingHeading Component (unchanged)
...
// Hero component (Updated background gradient)
const Hero: React.FC = () => (
  // Updated gradient: light gray -> white -> light gray (light mode)
  //                  dark gray -> black -> dark gray (dark mode)
  <section id="home" className="min-h-screen flex items-center justify-center text-center bg-gradient-to-br from-gray-100 via-white to-gray-200 dark:from-gray-900 dark:via-black dark:to-gray-800 px-4 py-20">
    <div className="max-w-4xl">
      <img
        src="https://i.postimg.cc/DwYRNRxG/4f2e8208dddaad092bb86fcb121ef352.jpg" // Gray placeholder bg
        alt="Your Name"
        className="w-32 h-32 md:w-40 md:h-40 rounded-full mx-auto mb-6 border-4 border-white dark:border-gray-700 shadow-lg"
        onError={(e) => {
      ...
      />
      {/* Use TypingHeading instead of HoverableText for h1 */}
      <TypingHeading
        text="HHi, I'm Ruchir" // Make sure this text is exactly as intended
        className="text-4xl md:text-6xl font-bold text-gray-900 dark:text-gray-100 mb-4 min-h-[1.2em]" // Added min-h for layout stability
        typingSpeed={80} // Adjust speed (ms per character)
      />
      {/* Keep HoverableText for the description or change if desired */}
      <HoverableText
        as="p"
        text="I'm a data scientist, passionate about turning data into impactful solutions through machine learning and intelligent systems." // Updated description
...
const App: React.FC = () => {
  ...
  return (
    // Apply the Outfit font family here
    <div className={`${isDarkMode ? 'dark' : ''} font-['Outfit',_sans-serif] bg-white dark:bg-black text-gray-900 dark:text-gray-100 transition-colors duration-300`}>
